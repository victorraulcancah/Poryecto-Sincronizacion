...
Si recibes un error 403 al intentar acceder a un endpoint de contabilidad:

#### 1. Verificar tus permisos actuales
...
```json
{
  "user": { "id": 1, "name": "Juan Pérez" },
  "roles": ["Contador"],
  "permissions": ["contabilidad.cajas.ver", "contabilidad.kardex.ver"],
...
{
  "permission": "contabilidad.cajas.ver"
}
```

#### 3. Ver todos los permisos disponibles
...
1. **Frontend usa permisos incorrectos**
   - ❌ `contabilidad.ver` (no existe)
   - ✅ `contabilidad.cajas.ver` (correcto)

2. **Usuario sin rol asignado**
   - Verificar que el usuario tenga un rol (Admin, Contador, Cajero, etc.)

3. **Rol sin permisos**
   - Ejecutar: `php artisan db:seed --class=ContabilidadPermissionsSeeder`

4. **Caché de permisos desactualizado**
   - Ejecutar: `php artisan permission:cache-reset`

### Guía Completa

Ver [TROUBLESHOOTING_PERMISOS.md](./TROUBLESHOOTING_PERMISOS.md) para una guía detallada.
