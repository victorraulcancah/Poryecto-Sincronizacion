...
export class ProductoModalComponent implements OnInit, OnChanges, OnDestroy {
  ...
  constructor(private fb: FormBuilder, private almacenService: AlmacenService) {
    this.productoForm = this.fb.group({
      ...
      marca_id: [''],
      // Sin `required`: estos cuatro campos ya no se muestran en el formulario
      // (los maneja el ERP), así que no pueden bloquear el guardado.
      precio_compra: [0, [Validators.min(0)]],
  ...
  resetearFormularios(): void {
    ...
    // Agregar elementos por defecto: los dos campos que siempre se llenan.
