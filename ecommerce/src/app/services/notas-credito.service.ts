...
export class NotasCreditoService {
  ...
  create(data: {
    ...
    return this.http.post<any>(this.apiUrl, data).pipe(
      catchError((err: any) => {
        const msg = err?.error?.error || err?.error?.message || 'Error al emitir Nota de Crédito';
  ...
  /**
   * Descargar XML de la nota de crédito
   */
  ...
  /**
   * Descargar CDR de la nota de crédito
   */
  ...
  /**
   * Editar nota de crédito (solo si estado = PENDIENTE)
   */
  update(id: number, data: {
    ...
    return this.http.put<any>(`${this.apiUrl}/${id}`, data).pipe(
      catchError((err: any) => {
        const msg = err?.error?.error || err?.error?.message || 'Error al editar Nota de Crédito';
        err.error = { ...(err.error || {}), message: msg };
        return throwError(() => err);
      })
    );
  ...
  /**
   * Generar XML de la nota de crédito (sin enviar a SUNAT)
   */
  ...
  /**
   * Enviar nota de crédito a SUNAT (requiere XML generado)
   */
